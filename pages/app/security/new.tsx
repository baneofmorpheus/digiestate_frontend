...
const CreateSecurity = () => {
  ...
  const createSecuritySchema = yup
    .object()
    .shape({
      phone_number: yup
        .string()
  ...
  const {
    ...
    resolver: yupResolver(createSecuritySchema),
    mode: 'all',
  });

  const router = useRouter();

  const createSecurity = async (data: UserType) => {
    try {
      setFormLoading(true);
      const securityInput = { ...data, role: 'security' };
      ...
      updateToastDispatch(
        updateToastData({
          ...
          detail: 'Security has been registered successfully.',
        })
      );
      return router.push(`/app/security`);
    } catch (error: any) {
  ...
  return (
    <div className=' pt-10 pl-2 pr-2'>
      <div className=' '>
        <PreviousPage label='New Security' />

        <form
          className='mb-4  ml-auto mr-auto lg:pr-0 lg:pl-0 pl-2 pr-2 '
          onSubmit={handleSubmit(createSecurity)}
        >
          <div className=' text-sm'>
            ...
            <p className='text-xs text-center text-gray-500'>
              The security will get their password sent to their email <br />{' '}
              with a verification link after you click the button below
            </p>
...
CreateSecurity.getLayout = function getLayout(page: NextPage) {
